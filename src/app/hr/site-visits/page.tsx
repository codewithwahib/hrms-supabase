...
export default function SiteVisitsPage() {
  ...
  // =====================================================
  // fetchVisits - UPDATED FOR SUPABASE
  // =====================================================
  ...
  const fetchVisits = useCallback(async () => {
    try {
      ...
      // ✅ Fetch all employees with their site_visits
      let query = supabase
        .from('employees')
      ...
      // ✅ Extract site visits from employees
      ...
      if (employeesData && employeesData.length > 0) {
        employeesData.forEach((emp: any) => {
          const siteVisits = emp.site_visits || []
          
          if (Array.isArray(siteVisits) && siteVisits.length > 0) {
            // Filter by date range
            let filteredVisits = siteVisits
            if (fromDate) {
              filteredVisits = filteredVisits.filter((v: any) => v.visitDate >= fromDate)
            }
            if (toDate) {
              filteredVisits = filteredVisits.filter((v: any) => v.visitDate <= toDate)
            }

            filteredVisits.forEach((visit: any) => {
              const visitKey = visit._key || `visit-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`
              ...
              allVisits.push({
                ...
                fromTime: visit.fromTime || '',
                toTime: visit.toTime || '',
                location: visit.location || '',
                followUps: visit.followUps || '',
                notes: visit.notes || '',
                liveLocation: visit.liveLocation || null,
                employee: {
                  id: emp.id,
              ...
              // Track unique values for meta
              if (visit.companyName) companies.add(visit.companyName)
              if (visit.customerName) customers.add(visit.customerName)
              if (emp.employee_id) employeesSet.add(emp.employee_id)
            })
          }
        })
      }

      // Sort by visitDate (newest first)
      allVisits.sort((a, b) => {
      ...
      setMeta({
        ...
        uniqueEmployees: employeesSet.size,
      ...
      const emps = new Map<string, {id: string, name: string, department: string}>()
      employeesData?.forEach((emp: any) => {
        if (emp.employee_id && emp.full_name) {
          emps.set(emp.employee_id, {
            id: emp.employee_id,
            name: emp.full_name,
            department: emp.department || ''
          })
  ...
  }, [selectedDepartment, selectedEmployee, fromDate, toDate]) // ✅ Removed supabase dependency
  ...
  // =====================================================
  // DELETE VISIT - UPDATED FOR SUPABASE
  // =====================================================
  ...
  const deleteVisit = async (employeeId: string, visitKey: string) => {
    ...
    try {
      ...
      // ✅ Remove the specific visit from the site_visits array
      const currentVisits = employeeData.site_visits || []
      const updatedVisits = currentVisits.filter((v: any) => {
        const vKey = v._key || v.visitKey
        return vKey !== visitKey
      })
          ...
          <div className="space-y-4">
            {visits.length === 0 ? (
              ...
              visits.map((visit, index) => {
                const isExpanded = expandedVisit === (visit._key || index.toString())
                const hasLiveLocation = visit.liveLocation && (visit.liveLocation.latitude || visit.liveLocation.longitude)
                const employeeName = visit.employee?.full_name || 'Unknown Employee'
                const employeeId = visit.employee?.employee_id || 'N/A'
                const employeeDepartment = visit.employee?.department || 'N/A'
                const employeePosition = visit.employee?.position || 'N/A'
                const employeePhone = visit.employee?.phone_number || 'N/A'
                      ...
                      <div className="border-t border-gray-100">
                        <div className="p-4">
                          ...
                          <div className="flex flex-wrap gap-3 pt-3 border-t border-gray-200">
                            <button
                              onClick={() => {
                                if (visit.employee?.employee_id) {
                                  deleteVisit(visit.employee.employee_id, visit._key)
                                } else {
                                  alert('Cannot delete: Employee ID not found')
                                }
                              }}
