...
export default function SiteVisitHistoryPage() {
  ...
  const applyFilters = useCallback(() => {
    if (!employee?.site_visits) {
      setFilteredVisits([])
      calculateStats([])
      return
    }

    let filtered = [...employee.site_visits]
  ...
  const fetchEmployeeData = useCallback(async () => {
    try {
      ...
      const enabled = transformedEmployee.enable_site_visits || false
      setIsSiteVisitEnabled(enabled)
      
      console.log('📌 Site visit enabled:', enabled)
      console.log('📌 Total visits:', transformedEmployee.site_visits?.length || 0)
  ...
  // =====================================================
  // checkSiteVisitStatus - useCallback
  // =====================================================
  ...
  const checkSiteVisitStatus = useCallback(async () => {
    ...
    try {
      ...
      // ✅ Fetch employee from Supabase to check status
      const { data, error: fetchError } = await supabase
        .from('employees')
        .select('enable_site_visits, site_visits')
        .eq('employee_id', employeeId)
        .maybeSingle()
      ...
      const enabled = data?.enable_site_visits || false
      setIsSiteVisitEnabled(enabled)
      ...
      if (data) {
        setEmployee((prev) => prev ? {
          ...prev,
          enable_site_visits: data.enable_site_visits,
          site_visits: data.site_visits || []
        } : null)
      }
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="bg-white shadow-sm p-4">
                ...
                <div className="text-xs text-gray-500 tracking-wider">ID: {employee.employee_id}</div>
              </div>
              <div className="bg-white shadow-sm p-4">
                <div className="flex items-center gap-2 text-sm text-gray-600">
                  <Building className="w-4 h-4 text-[#0071BD]" />
                ...
                <div className="text-xs text-gray-500 tracking-wider">{employee.position || 'N/A'}</div>
              </div>
              <div className="bg-white shadow-sm p-4">
                <div className="flex items-center gap-2 text-sm text-gray-600">
                  <Phone className="w-4 h-4 text-[#0071BD]" />
                ...
                <div className="text-xs text-gray-500 tracking-wider">{employee.cnic_number || 'N/A'}</div>
              </div>
              <div className="bg-white shadow-sm p-4">
                <div className="flex items-center gap-2 text-sm text-gray-600">
                  <MapPin className="w-4 h-4 text-[#0071BD]" />
            ...
            <div className="bg-white shadow-sm p-4 mb-6">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="text-sm text-gray-500 tracking-wider">
                  Showing {filteredVisits.length} of {employee?.site_visits?.length || 0} total
                </div>
              </div>
            </div>
            ...
            {filteredVisits.length > 0 && (
              <div className="mt-6 bg-white shadow-sm p-4">
                <div className="flex flex-wrap items-center justify-between text-sm text-gray-600 tracking-wider">
                  <div className="tracking-wider">
                    Showing {filteredVisits.length} records for {employee.full_name}
                    {(fromDate || toDate) && ` (${formatDateForDisplay(fromDate)} to ${formatDateForDisplay(toDate)})`}
                  </div>
                  <div className="flex flex-wrap items-center gap-4">
                    <span className="flex items-center gap-2 tracking-wider">
