...
export default function HRLeaveManagementPage() {
  ...
  // =====================================================
  // GET LEAVES - UPDATED FOR SUPABASE (FIXED)
  // =====================================================
  ...
  const fetchLeaves = useCallback(async () => {
    try {
      ...
      // ✅ Fetch all employees with their leaves from Supabase
      ...
      // ✅ Extract leaves from employees
      let allLeaves: LeaveRecord[] = []

      if (employeesData && employeesData.length > 0) {
        employeesData.forEach((emp: any) => {
          if (emp.leaves && Array.isArray(emp.leaves) && emp.leaves.length > 0) {
            emp.leaves.forEach((leave: any) => {
              const leaveKey = leave._key || `leave-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`
              ...
              allLeaves.push({
                ...
                reason: leave.reason || '',
                status: leave.status || 'pending',
                appliedOn: leave.appliedOn || new Date().toISOString(),
                adminRemarks: leave.adminRemarks || '',
                updatedOn: leave.updatedOn || '',
                employeeRef: emp.id || ''
              })
      ...
      console.log('Normalized leaves:', allLeaves)
  ...
  }, [updateStats]) // ✅ Removed supabase dependency
  ...
  // =====================================================
  // APPROVE / REJECT - UPDATED FOR SUPABASE
  // =====================================================
  ...
  const handleStatusUpdate = async (
    ...
    try {
      ...
      const currentLeaves = employeeData.leaves || []
      const updatedLeavesData = currentLeaves.map((l: any) => {
        const lKey = l._key || l.leaveKey
        if (lKey === leaveKey) {
          return {
            ...l,
            status: status,
      ...
      // ✅ Update local state
      const updatedLeavesList = leaves.map((item) => {
        const itemKey = item.leaveKey || item._key
        if (itemKey === leaveKey) {
          return {
            ...item,
      ...
      setLeaves(updatedLeavesList)
      updateStats(updatedLeavesList)
  ...
  const handleDeleteLeave = async (leave: LeaveRecord) => {
    ...
    try {
      ...
      // ✅ Remove the specific leave from the leaves array
      const currentLeaves = employeeData.leaves || []
      const updatedLeavesData = currentLeaves.filter((l: any) => {
        const lKey = l._key || l.leaveKey
        return lKey !== leaveKey
      })
      ...
      // ✅ Remove from local state
      const updatedLeavesList = leaves.filter((item) => {
        const itemKey = item.leaveKey || item._key
        return itemKey !== leaveKey
      })
