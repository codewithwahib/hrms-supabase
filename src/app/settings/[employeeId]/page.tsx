...
export default function SettingsPage() {
  ...
  // =====================================================
  // loadEmployee - UPDATED FOR SUPABASE
  // =====================================================
  ...
  const loadEmployee = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: fetchError } = await supabase
        .from('employees')
        .select('id, employee_id, full_name, department, position, username, password')
      ...
      setUsername(transformedEmployee.username || '');
    } catch (error) {
      console.error('Load settings error:', error);
      setError(error instanceof Error ? error.message : 'Failed to load settings');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data) {
        ...
        setUsername(updatedEmployee.username || username.trim());
      }
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="bg-gray-50 p-4 border-l-4 border-[#0071BD]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm tracking-wide">
                <div>
                  <p className="text-gray-500">Full Name</p>
                  <p className="font-medium text-gray-800 mt-1">{employee.full_name || '-'}</p>
                </div>
                <div>
                  <p className="text-gray-500">Employee ID</p>
                  <p className="font-medium text-gray-800 mt-1">{employee.employee_id || employeeId}</p>
                </div>
                <div>
                  <p className="text-gray-500">Department</p>
                  <p className="font-medium text-gray-800 mt-1 flex items-center gap-1">
