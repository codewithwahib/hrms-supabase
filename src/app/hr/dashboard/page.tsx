...
export default function HRDashboardPage() {
  ...
  // =====================================================
  // fetchDashboardData - useCallback (FIXED - removed supabase dependency)
  // =====================================================
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      // ✅ Extract leave requests from employees
      let allLeaveRequests: any[] = []
      if (employeesData && employeesData.length > 0) {
        employeesData.forEach((emp: any) => {
          if (emp.leaves && Array.isArray(emp.leaves)) {
            const leavesWithEmployee = emp.leaves.map((leave: any) => ({
              ...leave,
            ...
            allLeaveRequests = [...allLeaveRequests, ...leavesWithEmployee]
          }
        })
      }
  ...
  }, [calculateStats, getRecentActivities]) // ✅ Removed supabase dependency

  // =====================================================
  // USE EFFECT - fetchDashboardData
  // =====================================================
  ...
  const getMonthlyAttendanceData = useCallback(() => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      data.employees.forEach((emp: any) => {
        const empCheckIns = emp.check_in || []
        if (Array.isArray(empCheckIns)) {
          empCheckIns.forEach((check: any) => {
            if (!check || !check.time) return
            const checkDate = new Date(check.time)
        ...
        <div className="bg-white shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {safeEmployees.slice(0, 5).map((employee: any) => {
                  const today = new Date().toISOString().split('T')[0]
                  ...
                  const isOnLeave = safeLeaveRequests.some((leave: any) => {
                    ...
                    const fromDate = new Date(leave.fromDate)
                    const toDate = new Date(leave.toDate)
                    return leave.employeeName === employee?.full_name &&
                           fromDate <= todayDate && toDate >= todayDate
                  })
                  ...
                  return (
                    <tr key={employee.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-2.5 py-1.5">
                        <button
                          onClick={() => window.location.href = `/hr/attendance?id=${employee.id}`}
                          className="text-[#0071BD] hover:text-[#005a96]"
                        >
                          <Eye className="w-3 h-3" />
                        </button>
